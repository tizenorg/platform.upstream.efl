...
struct _Ecore_Buffer_Provider
{
   ...
   struct
   {
      void  (*consumer_add)      (Ecore_Buffer_Provider *provider,
                                  int queue_size, int w, int h, void *data);
...
static void     _ecore_buffer_provider_cb_consumer_disconnected(void *data,
                                                                struct es_provider *es_provider);
static void     _ecore_buffer_provider_cb_add_buffer(void *data, struct es_provider *es_provider,
                                                     struct es_buffer *buffer, uint32_t serial);
static Shared_Buffer  *_ecore_buffer_provider_shared_buffer_new(Ecore_Buffer_Provider *pvdr,
                                                                Ecore_Buffer *buffer);
static void     _ecore_buffer_provider_shared_buffer_free(Ecore_Buffer_Provider *pvdr,
                                                          Shared_Buffer *sb);
static void     _ecore_buffer_provider_cb_buffer_free(Ecore_Buffer* buf, void *data);

struct es_provider_listener _ecore_buffer_provider_listener =
{
...
ecore_buffer_provider_new(const char *name)
{
   ...
   es_provider_set_user_data(pvdr->provider, pvdr);
...
ecore_buffer_provider_free(Ecore_Buffer_Provider *pvdr)
{
   ...
   es_provider_destroy(pvdr->provider);
   free(pvdr);
}
...
ecore_buffer_provider_buffer_acquire(Ecore_Buffer_Provider *pvdr, Ecore_Buffer **ret_buf)
{
   ...
   if (!(ebq = pvdr->ebq)) return ret_flag;
...
ecore_buffer_provider_buffer_enqueue(Ecore_Buffer_Provider *pvdr, Ecore_Buffer *buffer)
{
        ...
        if (pvdr->free_slot > 0)
          {
             ...
             if (!sb)
               {
                  ERR("Unkown error occured - Failed to attach buffer: buffer %p", buffer);
                  return EINA_FALSE;
               }
             pvdr->free_slot--;
          }
          ...
          {
             WARN("No Free slot in Queue. Need to ecore_buffer_free of submitted buffer first.");
             return EINA_FALSE;
          }
     }
   ...
   es_provider_enqueue_buffer(pvdr->provider, _shared_buffer_proxy_get(sb), 0); // serial?
...
EAPI void
ecore_buffer_provider_consumer_add_cb_set(Ecore_Buffer_Provider *provider,
...
EAPI void
ecore_buffer_provider_consumer_del_cb_set(Ecore_Buffer_Provider *provider,
...
EAPI void
ecore_buffer_provider_buffer_released_cb_set(Ecore_Buffer_Provider *provider,
...
static void
_ecore_buffer_provider_cb_consumer_connected(void *data, struct es_provider *es_provider EINA_UNUSED,
                                             int32_t queue_size, int32_t width, int32_t height)
{
   Ecore_Buffer_Provider *pvdr = (Ecore_Buffer_Provider *)data;

   EINA_SAFETY_ON_NULL_RETURN(pvdr);

   DBG("Consumer Connected - queue_size %d, size (%dx%d)", queue_size, width, height);
...
static void
_ecore_buffer_provider_cb_consumer_disconnected(void *data, struct es_provider *es_provider EINA_UNUSED)
{
   Ecore_Buffer_Provider *pvdr = (Ecore_Buffer_Provider *)data;
...
static void
_ecore_buffer_provider_cb_add_buffer(void *data, struct es_provider *es_provider EINA_UNUSED,
...
_ecore_buffer_provider_shared_buffer_new(Ecore_Buffer_Provider *pvdr, Ecore_Buffer *buffer)
{
   ...
   EINA_SAFETY_ON_NULL_RETURN_VAL(pvdr, NULL);
   ...
   if (!(es_buffer = es_provider_attach_buffer(pvdr->provider, engine, w, h, format, flags)))
     {
   ...
   switch (export_type)
     {
      case EXPORT_TYPE_ID:
         es_provider_set_buffer_id(pvdr->provider, es_buffer, export_id, 0, 0, 0, 0, 0, 0);
         break;
      case EXPORT_TYPE_FD:
         es_provider_set_buffer_fd(pvdr->provider, es_buffer, export_id, 0, 0, 0, 0, 0, 0);
         break;
      default:
         break;
     }

   sb = _shared_buffer_new(es_buffer, w, h, format, flags);
   _shared_buffer_buffer_set(sb, buffer);
   _shared_buffer_state_set(sb, SHARED_BUFFER_STATE_ACQUIRE);

   es_buffer_set_user_data(es_buffer, sb);
...
{
   struct es_buffer *proxy = _shared_buffer_proxy_get(sb);

   EINA_SAFETY_ON_NULL_RETURN(pvdr);
   EINA_SAFETY_ON_NULL_RETURN(pvdr->ebq);
   EINA_SAFETY_ON_NULL_RETURN(sb);
   ...
   es_provider_detach_buffer(pvdr->provider, proxy);
   es_buffer_destroy(proxy);
