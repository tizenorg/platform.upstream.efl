...
#ifdef _WIN32
# define ECORE_WIN_ENGINE_NAME "module.dll"
#else
# define ECORE_WIN_ENGINE_NAME "module.so"
#endif


Eina_Module *
_ecore_win_engine_load(const char *engine)
{
   const char *path;
   Eina_List *l;
   Eina_Module *em = NULL;
   ...
   EINA_LIST_FOREACH(_engines_paths, l, path)
     {
             ...
             if (run_in_tree)
               {
                  ...
                  snprintf(tmp, sizeof(tmp), "%s/%s/.libs/%s",
                           path, engine, ECORE_WIN_ENGINE_NAME);
                  if (stat(tmp, &st) != 0)
                  tmp[0] = '\0';
               }
          }
        ...
        if (tmp[0] == '\0')
          snprintf(tmp, sizeof(tmp), "%s/%s/%s/%s",
                   path, engine, MODULE_ARCH, ECORE_WIN_ENGINE_NAME);
...
//   _registered_engines = eina_hash_string_small_new(EINA_FREE_CB(eina_module_free));
   ...
   if (getuid() == geteuid())
#endif
     {
        if (getenv("EFL_RUN_IN_TREE"))
          {
             struct stat st;
             const char mp[] = PACKAGE_BUILD_DIR"/src/modules/ecore_win/engines/";
             if (stat(mp, &st) == 0)
               {
   ...
   paths[0] = eina_module_symbol_path_get(_ecore_win_engine_init, "/ecore_win/engines");
#ifndef _WIN32
   /* 3. PREFIX/ecore_evas/engines/ */
   paths[1] = strdup(PACKAGE_LIB_DIR "/ecore_win/engines");
#else
   paths[1] = eina_module_symbol_path_get(_ecore_win_engine_init, "/../lib/ecore_win/engines");
#endif
...
void
_ecore_win_engine_shutdown(void)
{
   char *path;
...
const Eina_List *
_ecore_win_available_engines_get(void)
{
        ...
        if (it)
          {
             EINA_ITERATOR_FOREACH(it, info)
               {
                  char tmp[PATH_MAX];
                  snprintf(tmp, sizeof (tmp), "%s/%s/" ECORE_WIN_ENGINE_NAME,
