...
_edje_part_mouse_up_cb(void *data, Evas *e EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info)
{
   ...
   // TIZEN_ONLY(20150709): When parent is moving entry needs not to update
   // cursor and above comment seems not be valid since when parent is moving selection
   // cannot be ended.
   ...
   if (strlen(preedit_string) > 0)
     {
        ...
        if ((rp->part->entry_mode == EDJE_ENTRY_EDIT_MODE_PASSWORD) &&
            _edje_password_show_last)
          {
             ...
             _edje_entry_hide_visible_password(ed, en->rp);
           // TIZEN_ONLY(20150629): In case of unicode language preedit status is needed to be maintained
           // so markup text comes from imf
