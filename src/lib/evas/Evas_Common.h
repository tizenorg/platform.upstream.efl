...
typedef enum _Evas_Display_Mode
{
   ...
   EVAS_DISPLAY_MODE_INHERIT = 1, /** deprecated **/
   EVAS_DISPLAY_MODE_COMPRESS = 2, /**< Use this mode when you want to give compress display mode hint to an object */
   EVAS_DISPLAY_MODE_EXPAND = 3, /**< Use this mode when you want to give expand display mode hint to an object */
   EVAS_DISPLAY_MODE_DONT_CHANGE = 4 /**< Use this mode when an object should not change its display mode */
   //
 ...
 *
 * @remark This function looks up a numeric return value for the named engine
 ...
 * invalid engine.
 *
 * @remark The programmer should NEVER rely on the numeric ID of an engine
 ...
 * @ingroup Evas_Output_Method
 *
 * @remark Calling this will return a handle (pointer) to an Evas linked
 ...
 * @ingroup Evas_Output_Method
 *
 * @remark When this function is called it will free the engine list passed in
 ...
 *
 * @remark This function removes the region from the render updates list. It
 ...
 *
 * @param[in] m map to change all 4 points (must be of size 4).
 * @param[in] obj object to use unmapped geometry to populate map coordinates.
 * @param[in] z Point Z Coordinate hint (pre-perspective transform). This value
 ...
 * function, avoiding code duplication all around.
 *
 * @remark Z Point coordinate is assumed as 0 (zero).
 *
 * @param[in] m map to change all 4 points (must be of size 4).
 * @param[in] obj object to use unmapped geometry to populate map coordinates.
 *
 ...
 * @param[in] w width to use to calculate second and third points.
 * @param[in] h height to use to calculate third and fourth points.
 * @param[in] z Point Z Coordinate hint (pre-perspective transform). This value
 *        will be used for all four points.
 *
 * @see evas_map_util_points_populate_from_object()
 ...
 *
 * @param[in] m map to change the color of.
 ...
 * @param[in] degrees amount of degrees from 0.0 to 360.0 to rotate.
 ...
 * @param[in] zoomy vertical zoom to use.
 * @param[in] cx zooming center horizontal position.
 * @param[in] cy zooming center vertical position.
 *
 * @see evas_map_point_coord_set()
 ...
 * @param[in] cx rotation's center horizontal position.
 * @param[in] cy rotation's center vertical position.
 * @param[in] cz rotation's center vertical position.
 */
 ...
 * with evas_map calls.
 *
 * @param[in] count number of points in the map.
 ...
 *
 * @param[in] m map to get the smooth from. Must not be NULL.
 ...
 *
 * @param[in] m map to modify. Must not be NULL.
 * @param[in] enabled enable or disable alpha map rendering
 */
 ...
 *
 * @param[in] m map to get the alpha from. Must not be NULL.
 ...
 *
 * @param[in] m map to copy. Must not be NULL.
 ...
 *
 * @remark The X and Y and Z coordinates are in canvas units. Z is optional and may
 ...
 *
 * @remark Remember all coordinates are canvas global ones like with move and resize
 * in evas.
 *
 * @param[in] m map to change point. Must not be @c NULL.
 * @param[in] idx index of point to change. Must be smaller than map size.
 * @param[in] x Point X Coordinate
 * @param[in] y Point Y Coordinate
 ...
 *
 * @param[in] m map to query point.
 * @param[in] idx index of point to query. Must be smaller than map size.
 * @param[out] x where to return the X coordinate.
 * @param[out] y where to return the Y coordinate.
 * @param[out] z where to return the Z coordinate.
 */
 ...
 * selection.
 *
 * @param[in] m map to change the point of.
 * @param[in] idx index of point to change. Must be smaller than map size.
 * @param[in] u the X coordinate within the image/texture source
 * @param[in] v the Y coordinate within the image/texture source
 *
 * @see evas_map_point_coord_set()
 * @see evas_object_map_set()
 * @see evas_map_util_points_populate_from_object_full()
 ...
 * @param[in] m map to query point.
 * @param[in] idx index of point to query. Must be smaller than map size.
 * @param[out] u where to write the X coordinate within the image/texture source
 * @param[out] v where to write the Y coordinate within the image/texture source
 */
 ...
 * of the map.
 *
 * @param[in] m map to get the color of the vertex from.
...
/**
 * @brief Returns the escaped version of the string.
...
/**
 * @brief Check if the current cursor position is a visible format. This way is more
...
/**
 * @brief Check if the current cursor position points to the terminating null of the
...
/**
 * @brief Adds format to the current cursor position. If the format being added is a
 * visible format, add it *before* the cursor position, otherwise, add it after.
 * This behavior is because visible formats are like characters and invisible
 * should be stacked in a way that the last one is added last.
 ...
 * @param[in] cur1 one side of the range.
 * @param[in] cur2 the other side of the range
 * @param[in] format The form on which to return the text. Markup - in textblock markup. Plain - UTF8.
...
/**
 * @brief Returns the geometry of the cursor. Depends on the type of cursor requested.
 ...
 *
 * @param[in] cur the cursor.
 * @param[out] cx the x of the cursor
 * @param[out] cy the y of the cursor
 * @param[out] cw the width of the cursor
 * @param[out] ch the height of the cursor
 * @param[out] dir the direction of the cursor, can be NULL.
 * @param[in] ctype the type of the cursor.
 ...
 * @param[out] cw the w of the char.
 ...
 * @param[out] cy the y of the line.
 ...
 *
 * @remark The returned value is @b not to be modified, just use it as your
 * parent class.
 */
 ...
 *
 * @remark This function converts a given color in HSV color format to RGB
 * color format.
 *
 * @ingroup Evas_Utils
 **/
 ...
 *
 * @remark This function converts a given color in RGB color format to HSV
 * color format.
 *
 * @ingroup Evas_Utils
 **/
 ...
 * @param[out] b The Blue component of the color.
 *
 * @remark This function pre-multiplies a given rgb triplet by an alpha
 ...
 * @param[out] b The Blue component of the color.
 *
 * @remark This function undoes pre-multiplication a given rbg triplet by an
 ...
 *
 * @remark This function pre-multiplies a given data by an alpha
 * factor. Alpha factor is used to define transparency.
 *
 * @ingroup Evas_Utils
 **/
 ...
 * @param[in] len  The length value.
 *
 * @remark This function undoes pre-multiplication of a given data by an alpha
 * factor. Alpha factor is used to define transparency.
 *
 * @ingroup Evas_Utils
 **/
 ...
 *
 * @remark The only boundary check done is that @p pos must be >= 0. Other than that,
 ...
 * @param[out] decoded Address where to store the decoded code point. Optional.
 *
 ...
 *
 * @remark The only boundary check done is that @p pos must be >= 1. Other than that,
 * no checks are performed, so passing an index value that's not within the
 * length of the string will result in undefined behavior.
 *
 * @param[in] str The UTF-8 string
 ...
 */
/**
 * @brief Checks the state of a given modifier key, at the time of the
...
/**
 * @brief Checks the state of a given lock key, at the time of the call. If
