...
void
evas_software_xcb_swapbuf_init(void)
{
}

void
evas_software_xcb_swapbuf_free(Outbuf *buf)
{
   evas_software_xcb_swapbuf_flush(buf, NULL, MODE_FULL);
   evas_software_xcb_swapbuf_idle_flush(buf);
   if (buf->priv.pal)
      evas_software_xlib_x_color_deallocate
      (buf->priv.x11.xlib.disp, buf->priv.x11.xlib.cmap,
       buf->priv.x11.xlib.vis, buf->priv.pal);
   evas_xcb_swapper_free(buf->priv.swapper);
   eina_array_flush(&buf->priv.onebuf_regions);
   free(buf);
}

Outbuf *
evas_software_xcb_swapbuf_setup_x(int w, int h, int rot, Outbuf_Depth depth,
                                  ...
                                  int shape_dither, int destination_alpha)
{
   ...
   if ((buf->rot == 0) || (buf->rot == 180))
      buf->priv.swapper = evas_xcb_swapper_new(buf->priv.x11.xlib.disp,
   ...
   if (evas_xcb_swapper_byte_order_get(buf->priv.swapper) == LSBFirst)
      buf->priv.x11.xlib.swap = 1;
   if (evas_xcb_swapper_bit_order_get(buf->priv.swapper) == MSBFirst)
      buf->priv.x11.xlib.bit_swap = 1;
#else
   if (evas_xcb_swapper_byte_order_get(buf->priv.swapper) == MSBFirst)
      buf->priv.x11.xlib.swap = 1;
   if (evas_xcb_swapper_bit_order_get(buf->priv.swapper) == MSBFirst)
      buf->priv.x11.xlib.bit_swap = 1;
#endif
   ...
   d = evas_xcb_swapper_depth_get(buf->priv.swapper);
   if (buf->priv.pal)
      {
...
void *
evas_software_xcb_swapbuf_new_region_for_update(Outbuf *buf, int x, int y, int w, int h, int *cx, int *cy, int *cw, int *ch)
{
         ...
         if (!im)
            {
               ...
               data = evas_xcb_swapper_buffer_map(buf->priv.swapper, &bpl,
                                                  &(ww), &(hh));
...
void
evas_software_xcb_swapbuf_free_region_for_update(Outbuf *buf EINA_UNUSED, RGBA_Image *update EINA_UNUSED)
{
   /* no need to do anything - they are cleaned up on flush */
}

void
evas_software_xcb_swapbuf_flush(Outbuf *buf, Tilebuf_Rect *rects EINA_UNUSED, Evas_Render_Mode render_mode)
{
   ...
   if (!buf->priv.pending_writes)
      {
         ...
         evas_xcb_swapper_buffer_unmap(buf->priv.swapper);
         // TIZEN_ONLY [[
         evas_xcb_swapper_buffer_size_get(buf->priv.swapper, &buf_w, &buf_h);
         ...
         evas_xcb_swapper_swap(buf->priv.swapper, result, n);
   ...
   else
      {
         ...
         if ((buf_w == buf->w) && (buf_h == buf->h)) // ]]
         evas_xcb_swapper_swap(buf->priv.swapper, result, n);
         //        evas_xcb_swapper_swap(buf->priv.swapper, NULL, 0);
      }
}

void
evas_software_xcb_swapbuf_idle_flush(Outbuf *buf EINA_UNUSED)
{
   return;
}
...
static void
__fbdump( unsigned char* shm_data, int w, int h, int bytes_per_line )
{
   static int frame = 0;
   unsigned char* data = NULL;
   char str_file[1024] = "/opt/usr/media/dump/";
   char str_num[1024] = "";

   sprintf( str_num, "%04d-%04d.tga", getpid(), frame );
   strcat( str_file, str_num );

   FILE* fp= fopen(str_file,"w+b");
   if(fp == NULL)
      {
         printf( "[XECO][FBDUMP] Unable to open %s for writing\n", str_file);
         return;
      }

   char TGA_Header[18];
   memset( TGA_Header, 0, sizeof(TGA_Header) );
   TGA_Header[2] = 2; //TGA_RGB;
   ...
   int numbytes = fwrite(TGA_Header,sizeof(TGA_Header),1,fp);
   int siz = w * h * 4;

   data = (unsigned char*)malloc( sizeof(unsigned char) * h * bytes_per_line );
   if( !data )
      {
         printf( "failed!!!!\n" );
         return;
      }
   memset( data, 0x0, h * bytes_per_line );

   int i = 0;
   int j = 0;
   for( i = h-1; i >= 0; i-- )
      {
         memcpy( data+(bytes_per_line*j), shm_data+(bytes_per_line*i), bytes_per_line);
         j++;
      }
   ...
   free( data );
   printf( "[evas_x11_yigl] save dump file:%s\n", str_file );
}


void
evas_software_xcb_swapbuf_push_updated_region(Outbuf *buf, RGBA_Image *update, int x, int y, int w, int h)
{
   Gfx_Func_Convert conv_func = NULL;
   ...
   if ((buf->rot == 0) || (buf->rot == 180))
      {
         dst_data = evas_xcb_swapper_buffer_map(buf->priv.swapper, &bpl,
                                                &(ww), &(hh));
         if (!dst_data) return;
         if (buf->rot == 0)
            {
...
void
evas_software_xcb_swapbuf_reconfigure(Outbuf *buf, int w, int h, int rot,
                                      Outbuf_Depth depth)
{
...
int
evas_software_xcb_swapbuf_get_rot(Outbuf * buf)
{
   return buf->rot;
}

Eina_Bool
evas_software_xcb_swapbuf_alpha_get(Outbuf *buf)
{
   return buf->priv.destination_alpha;
}

Render_Engine_Swap_Mode
evas_software_xcb_swapbuf_buffer_state_get(Outbuf *buf)
{
   Render_Engine_Swap_Mode mode;
   if (!buf->priv.swapper) return MODE_FULL;
   mode = evas_xcb_swapper_buffer_state_get(buf->priv.swapper);
   return mode;
}
